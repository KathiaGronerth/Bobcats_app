    ...
    <div className="ride-card">
      <div className="ride-card-container">
        <p className="ride-detail" style={{ fontSize: "16px" }}>
          <IoPerson style={{ paddingRight: "5px" }} /> Passenger :{" "}
          {ride.passenger.name}
        </p>
      </div>
...
const DriverHistory = () => {
  const [rides, setRides] = useState({ passenger: [] });
    ...
    <div className="ride-page">
      ...
      <div className="ride-container">
        {rides.passenger.map((ride) => (
          <Card key={ride.ride.id} ride={ride} />
        ))}
      </div>
    </div>
  );
